...
public class SimpleItemItemScorer extends AbstractItemScorer {
    ...
    public void score(long user, @Nonnull MutableSparseVector scores) {
        ...
        for (VectorEntry e: scores.fast(VectorEntry.State.EITHER)) {
            ...
            // Sum all scores of all items in the neighborhood and compute
            int count = 0;
            double weight = 0.0;
            double score = 0.0;
            for (ScoredId neighbor: neighbors) {
                ...
                if (ratings.containsKey(neighbor.getId())) {
                    double currWeight = neighbor.getScore();
                    score += ratings.get(neighbor.getId()) * currWeight;
                    weight += currWeight;
                    count++;
                }
            }
            scores.set(item, score/weight);
        }
    }
